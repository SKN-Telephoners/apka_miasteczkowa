...
const NotificationCard: React.FC<NotificationCardProps> = ({ notification, onPress, actions, isExpanded, isSubItem }) => {
    ...
    return (
        <TouchableOpacity
            style={[
                styles.container,
                { backgroundColor: isSubItem ? 'transparent' : colors.background, borderColor: colors.border },
                !notification.is_read && !isSubItem && { borderLeftWidth: 4, borderLeftColor: colors.primary },
                isSubItem && {
                ...
                {isAggregated && (
                    <View style={{ justifyContent: 'center', paddingLeft: 8 }}>
                        <View style={{ transform: [{ rotate: isExpanded ? '180deg' : '0deg' }] }}>
                            <AppIcon name="ArrowDown" size={20} color={colors.icon} />
                        </View>
                    </View>
                )}
            </View>
