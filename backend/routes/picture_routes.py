...
from backend.picture_helpers import validate_and_process_image, upload_to_r2
import magic
import os
...
def upload_file():
    ...
    s3_key = upload_to_r2(processed_file, folder)
...
def upload_multiple_files():
    ...
    for file in files:
        ...
        s3_key = upload_to_r2(processed_file, "events")
        
        if s3_key:
            r2_base_url = current_app.config.get("R2_PUBLIC_URL", "").rstrip('/')
            uploaded_data.append({
                "picture_url": f"{r2_base_url}/{s3_key}",
                "cloud_id": s3_key,
                "public_id": s3_key
            })
    ...
    if not uploaded_data and errors:
        current_app.logger.error(f"ERROR: /upload_multiple_files, s3 upload error for all files:")
        current_app.logger.exception(errors, stack_info=True)
