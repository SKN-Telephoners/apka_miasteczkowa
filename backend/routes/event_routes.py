...
def create_event():
    ...
    required_keys = {"name", "description", "date", "time", "location", "tags", "typeof"}
    ...
    tags = sanitize_input(str(event_data.get("tags", ""))).strip()
    typeof = sanitize_input(str(event_data.get("typeof", ""))).strip()
    ...
    if not typeof in Constants.EVENT_TYPES:
        return make_api_response(ResponseTypes.INVALID_DATA, message="Invalid type of event")

    if len(tags) > Constants.MAX_TAGS_LEN:
        return make_api_response(ResponseTypes.INVALID_DATA, message="Too many tags")
...
def edit_event(event_id):
    ...
    if raw_tags is not None:
        tags = sanitize_input(str(raw_tags)).strip()
        if len(tags) > Constants.MAX_TAGS_LEN:
            return make_api_response(ResponseTypes.INVALID_DATA, message="Too many tags")
        event.tags = tags
    
    raw_typeof = event_data.get("typeof")
    if raw_typeof is not None:
        typeof = sanitize_input(str(raw_typeof)).strip()
        if not typeof in Constants.EVENT_TYPES:
            return make_api_response(ResponseTypes.INVALID_DATA, message="Invalid type of event")
        event.typeof = typeof
