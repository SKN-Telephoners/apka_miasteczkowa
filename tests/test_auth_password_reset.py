...
def test_password_reset(client, registered_user, app):
    with app.app_context():
        ...
        with mail.record_messages() as outbox:
            response = client.post("/api/email/reset_password_request", json=email_payload)

            assert response.status_code == 200
            assert len(outbox) == 1
            assert outbox[0].subject == 'Reset password'

            match = search(r'(http://.+/api/email/reset_password/\S+)', outbox[0].body)
            assert match, "No reset URL found in email body"

            reset_url = match.group()
            token_only = reset_url.split('/')[-1]
            token_path = f"/api/email/reset_password/{token_only}"
        ...
        response = client.post("/api/auth/login", json={"username": user.username, "password": "Dupa!123"})
        assert response.status_code == 200
...
def test_password_reset_revokes_all_tokens(client, registered_user, app):
    with app.app_context():
        ...
        with mail.record_messages() as outbox:
            client.post("/api/email/reset_password_request", json=email_payload)

            match = search(r'/api/email/reset_password/([a-zA-Z0-9\-\._]+)', outbox[0].body)
            assert match, "Reset path not found in email"
            token_path = f"/api/email/reset_password/{match.group(1)}"
        ...
        protected_response = client.get("api/users/me", headers={"Authorization": f"Bearer {initial_access_token}"})
        assert protected_response.status_code == 401
        
        refresh_response = client.post("api/auth/refresh", headers={"Authorization": f"Bearer {initial_refresh_token}"})
        assert refresh_response.status_code == 401

        login_response_new = client.post("/api/auth/login", json={"username": user.username, "password": "NewSecret123"})
